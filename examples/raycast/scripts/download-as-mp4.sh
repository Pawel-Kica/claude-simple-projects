...
# Documentation:
# @raycast.description Download video from the clipboard URL as MP4 via yt-dlp to Downloads

# Raycast runs script commands with a bare PATH (no login shell), so ~/.local/bin
# (yt-dlp nightly) and /opt/homebrew/bin (ffmpeg, needed for the merge) are missing.
# Nightly must win over brew's stale copy or every adaptive format 403s.
export PATH="$HOME/.local/bin:/opt/homebrew/bin:/usr/local/bin:$PATH"

YTDLP="$(command -v yt-dlp)"
LOG="/tmp/raycast-download-mp4.log"
...
(
  ...
  if [[ -n "$FILEPATH" ]]; then
    ...
    osascript -e "display notification \"Failed, see $LOG\" with title \"Download as MP4\""
  fi
) &
disown
